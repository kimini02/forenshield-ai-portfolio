...
GMFLOW_MODEL_ID = "gmflow"
GMFLOW_MODEL_NAME = "GMFlow"
GMFLOW_ROLE = "MOTION_AUXILIARY"
GMFLOW_AUXILIARY_STATUS = "AUXILIARY_ONLY"
GMFLOW_QUALITY_AVAILABLE_STATUS = "AVAILABLE"
GMFLOW_INSUFFICIENT_STATUS = "INSUFFICIENT_FLOW_QUALITY"
GMFLOW_SCORE_MEANING = "MOTION_INSTABILITY_NOT_FAKE_PROBABILITY"
GMFLOW_EXPLANATION = (
    "GMFlow는 딥페이크 확률이 아니라 프레임 간 얼굴 움직임 불안정성을 분석하는 보조 지표입니다. "
    "이 값은 최종 위험도 산출에 직접 반영되지 않으며, Xception과 TimeSformer 결과를 해석하는 참고 근거로 사용됩니다."
)
GMFLOW_LIMITATIONS = [
...
FLOW_QUALITY_GATE_REQUIRED_FIELDS = [
    "face_valid_ratio",
    "bbox_jitter",
    "flow_mag_std",
    "angle_dispersion",
]
MIN_FLOW_PAIRS_FOR_QUALITY = 4
MIN_DURATION_SEC_FOR_QUALITY = 2.0
MAX_BBOX_JITTER_FOR_QUALITY = 0.25
...
def read_video_metadata(video_path: Path) -> dict[str, Any]:
    cap = cv2.VideoCapture(str(video_path))
    if not cap.isOpened():
        return {"duration": None, "fps": None, "frame_count": None}
...
def build_flow_quality_metadata(
    *,
    video_metadata: dict[str, Any] | None = None,
    face_valid_ratio: float | None = None,
    num_flow_pairs: int | None = None,
    bbox_jitter: float | None = None,
    flow_mag_mean: float | None = None,
    flow_mag_std: float | None = None,
    temporal_jitter: float | None = None,
    spatial_inconsistency: float | None = None,
...
def normalize_flow_quality_metadata(metadata: dict[str, Any] | None) -> dict[str, Any]:
    metadata = dict(metadata or {})
    return {field: metadata.get(field) for field in FLOW_QUALITY_METADATA_FIELDS}


def missing_quality_metadata_fields(metadata: dict[str, Any] | None) -> list[str]:
    normalized = normalize_flow_quality_metadata(metadata)
    return [field for field in FLOW_QUALITY_METADATA_FIELDS if normalized.get(field) is None]


def quality_notes(metadata: dict[str, Any] | None) -> list[str]:
    missing = missing_quality_metadata_fields(metadata)
    if not missing:
        return []
    return [
        "현재 품질 판단 제한: "
        + ", ".join(missing)
        + " 값이 null이므로 GMFlow quality gate를 완전하게 적용할 수 없습니다."
    ]


def processing_status_from_report_status(status: Any) -> str:
    return "OK" if str(status or "").lower() == "ok" else "FAILED"


def motion_instability_level(score: float | None) -> str | None:
    score_value = _as_float_or_none(score)
...
def evaluate_flow_quality(metadata: dict[str, Any] | None) -> dict[str, Any]:
    metadata = normalize_flow_quality_metadata(metadata)
    reasons: list[str] = []

    face_valid_ratio = _as_float_or_none(metadata.get("face_valid_ratio"))
    if face_valid_ratio is not None and face_valid_ratio < 0.75:
        reasons.append("LOW_FACE_VALID_RATIO")

    num_flow_pairs = metadata.get("num_flow_pairs")
    if num_flow_pairs is not None:
        try:
            if int(num_flow_pairs) < MIN_FLOW_PAIRS_FOR_QUALITY:
                reasons.append("TOO_FEW_FLOW_PAIRS")
        except (TypeError, ValueError):
            reasons.append("INVALID_FLOW_PAIR_COUNT")

    bbox_jitter = _as_float_or_none(metadata.get("bbox_jitter"))
    if bbox_jitter is not None and bbox_jitter > MAX_BBOX_JITTER_FOR_QUALITY:
        reasons.append("EXCESSIVE_BBOX_JITTER")

    duration = _as_float_or_none(metadata.get("duration"))
    if duration is not None and duration < MIN_DURATION_SEC_FOR_QUALITY:
        reasons.append("SHORT_DURATION")

    quality_status = GMFLOW_INSUFFICIENT_STATUS if reasons else GMFLOW_QUALITY_AVAILABLE_STATUS
    return {"qualityStatus": quality_status, "reasons": reasons}


def build_gmflow_auxiliary_evidence(
    *,
    raw_score: float | None,
    quality_metadata: dict[str, Any] | None = None,
    motion_timeline: list[dict[str, Any]] | None = None,
    suspicious_segments: list[dict[str, Any]] | None = None,
    processing_status: str = "OK",
) -> dict[str, Any]:
    normalized_quality_metadata = normalize_flow_quality_metadata(quality_metadata)
    quality = evaluate_flow_quality(normalized_quality_metadata)
    score = round(float(raw_score), 6) if raw_score is not None else None
    missing_quality_fields = missing_quality_metadata_fields(normalized_quality_metadata)
    notes = quality_notes(normalized_quality_metadata)
    return {
        "modelName": GMFLOW_MODEL_NAME,
        "role": GMFLOW_ROLE,
        "processingStatus": processing_status,
        "gmflowStatus": GMFLOW_AUXILIARY_STATUS,
        "qualityStatus": quality["qualityStatus"],
        "contributesToFinalScore": False,
        "gmflowRawScore": score,
        "motionInstabilityScore": score,
        "motionInstabilityLevel": motion_instability_level(score),
        "scoreMeaning": GMFLOW_SCORE_MEANING,
        "display": dict(GMFLOW_DISPLAY),
        "explanation": GMFLOW_EXPLANATION,
        "limitations": list(GMFLOW_LIMITATIONS),
        "notes": notes,
        "qualityMetadata": normalized_quality_metadata,
        "qualityMetadataMissing": missing_quality_fields,
        "qualityGateReasons": quality["reasons"],
        "motionTimeline": motion_timeline or [],
        "suspiciousMotionSegments": suspicious_segments or [],
    }


def is_gmflow_model(model_name: str | None) -> bool:
    return str(model_name or "").lower() == GMFLOW_MODEL_ID


def attach_gmflow_auxiliary_fields(report: dict[str, Any], raw_score: float | None = None) -> dict[str, Any]:
    if not is_gmflow_model(report.get("model")):
        return report

    score = report.get("motion_anomaly_score") if raw_score is None else raw_score
    quality_metadata = report.get("quality_metadata") or report.get("qualityMetadata") or {}
    motion_timeline = report.get("motionTimeline") or []
    suspicious_segments = report.get("suspiciousMotionSegments") or []
    evidence = build_gmflow_auxiliary_evidence(
        raw_score=score,
        quality_metadata=quality_metadata,
        motion_timeline=motion_timeline,
        suspicious_segments=suspicious_segments,
        processing_status=processing_status_from_report_status(report.get("status")),
    )
    report["modelName"] = GMFLOW_MODEL_NAME
    report["role"] = GMFLOW_ROLE
    report["processingStatus"] = evidence["processingStatus"]
    report["gmflowStatus"] = evidence["gmflowStatus"]
    report["qualityStatus"] = evidence["qualityStatus"]
    report["contributesToFinalScore"] = False
    report["gmflowRawScore"] = evidence["gmflowRawScore"]
    report["motionInstabilityScore"] = evidence["motionInstabilityScore"]
    report["motionInstabilityLevel"] = evidence["motionInstabilityLevel"]
    report["scoreMeaning"] = evidence["scoreMeaning"]
    report["display"] = evidence["display"]
    report["explanation"] = evidence["explanation"]
    report["limitations"] = evidence["limitations"]
    report["notes"] = evidence["notes"]
    report["qualityMetadata"] = evidence["qualityMetadata"]
    report["qualityMetadataMissing"] = evidence["qualityMetadataMissing"]
    report["qualityGateReasons"] = evidence["qualityGateReasons"]
    report["motionTimeline"] = evidence["motionTimeline"]
    report["suspiciousMotionSegments"] = evidence["suspiciousMotionSegments"]
    if report.get("pred_label") is not None:
        report["motionHeuristicLabel"] = report.get("pred_label")
        report["predictionUsage"] = "DIAGNOSTIC_ONLY"
    report["gmflowAuxiliaryEvidence"] = evidence
    return report
...
def _build_interpretation(
    ...
    summary = (
        f"motionInstabilityScore={score:.4f} (diagnostic threshold={threshold}); "
        "GMFlow is auxiliary motion instability evidence, not a fake/real probability."
    )
...
def analyze_video_detailed(
    ...
    base = {
        ...
        "quality_metadata": build_flow_quality_metadata(video_metadata=video_metadata),
    }
    try:
        ...
        quality_metadata = build_flow_quality_metadata(
            video_metadata=video_metadata,
            num_flow_pairs=len(per_pair),
            flow_mag_mean=aggregate["flow_mag_mean"],
            flow_mag_std=aggregate["flow_mag_std"],
            temporal_jitter=aggregate["temporal_jitter"],
            spatial_inconsistency=aggregate["spatial_inconsistency_mean"],
            angle_dispersion=aggregate["angle_dispersion_mean"],
        )
        ...
        attach_gmflow_auxiliary_fields(base, raw_score=None)
        return base
...
def normalize_report_schema(report: dict[str, Any]) -> dict[str, Any]:
    ...
    video_metadata = report.get("video_metadata") or {}
    existing_quality_metadata = report.get("quality_metadata") or report.get("qualityMetadata") or {}
    quality_metadata = build_flow_quality_metadata(
        video_metadata=video_metadata,
        face_valid_ratio=existing_quality_metadata.get("face_valid_ratio"),
        num_flow_pairs=internal_aggregate["frame_pairs"],
        bbox_jitter=existing_quality_metadata.get("bbox_jitter"),
        flow_mag_mean=flow_mag_mean,
        flow_mag_std=flow_mag_std,
        temporal_jitter=internal_aggregate["temporal_jitter"],
        spatial_inconsistency=internal_aggregate["spatial_inconsistency_mean"],
        angle_dispersion=internal_aggregate["angle_dispersion_mean"],
    )
    report["quality_metadata"] = quality_metadata
    report["qualityMetadata"] = quality_metadata
...
def enrich_reports_with_scores(
    ...
    for report in reports:
        if report.get("status") != "ok":
            ...
            report["motionTimeline"] = []
            report["suspiciousMotionSegments"] = []
            attach_gmflow_auxiliary_fields(report, raw_score=None)
            continue

        # ?곸꽭(score_breakdown) ?덉쑝硫?湲곗〈 濡쒖쭅 洹몃?濡??ъ슜
        if "score_breakdown" in report:
            aggregate = report["score_breakdown"]["aggregate"]
            if not report.get("quality_metadata"):
                existing_quality_metadata = report.get("qualityMetadata") or {}
                quality_metadata = build_flow_quality_metadata(
                    video_metadata=report.get("video_metadata") or {},
                    face_valid_ratio=existing_quality_metadata.get("face_valid_ratio"),
                    num_flow_pairs=aggregate.get("frame_pairs") or report.get("frame_pairs_used"),
                    bbox_jitter=existing_quality_metadata.get("bbox_jitter"),
                    flow_mag_mean=aggregate.get("flow_mag_mean"),
                    flow_mag_std=aggregate.get("flow_mag_std"),
                    temporal_jitter=aggregate.get("temporal_jitter"),
                    spatial_inconsistency=aggregate.get("spatial_inconsistency_mean"),
                    angle_dispersion=aggregate.get("angle_dispersion_mean"),
                )
                report["quality_metadata"] = quality_metadata
                report["qualityMetadata"] = quality_metadata
            ...
            motion_timeline = build_motion_timeline(report, cohort, threshold)
            report["motionTimeline"] = motion_timeline
            report["suspiciousMotionSegments"] = top_suspicious_motion_segments(motion_timeline)
            attach_gmflow_auxiliary_fields(report, raw_score=report["motion_anomaly_score"])
            continue
        ...
        report["pred_label"] = "fake" if score >= threshold else "real"
        existing_quality_metadata = report.get("quality_metadata") or report.get("qualityMetadata") or {}
        quality_metadata = existing_quality_metadata or build_flow_quality_metadata(
            video_metadata=report.get("video_metadata") or {},
            face_valid_ratio=existing_quality_metadata.get("face_valid_ratio"),
            num_flow_pairs=report.get("frame_pairs_used") or report.get("frame_pairs"),
            bbox_jitter=existing_quality_metadata.get("bbox_jitter"),
            flow_mag_mean=report.get("flow_mean"),
            flow_mag_std=report.get("flow_std"),
        )
        report["quality_metadata"] = quality_metadata
        report["qualityMetadata"] = quality_metadata
...
def _run_metrics(reports: list[dict[str, Any]], cohort: dict[str, float]) -> dict[str, Any]:
    ...
    return {
        "schema_version": SCHEMA_VERSION,
        "method": "optical_flow_motion_heuristic",
        "note": "GMFlow/optical-flow score is auxiliary motion instability evidence; threshold metrics are diagnostic only.",
        "count": len(reports),
        "ok": len(ok),
...
def _segment_metric_mean(segment: dict[str, Any], key: str) -> float | None:
    metric = (segment.get("aggregate") or {}).get(key) or {}
    return _as_float_or_none(metric.get("mean"))


def _frame_to_ms(frame_index: Any, fps: Any) -> int | None:
    frame = _as_float_or_none(frame_index)
    fps_value = _as_float_or_none(fps)
    if frame is None or fps_value is None or fps_value <= 0:
        return None
    return int(round((frame / fps_value) * 1000.0))


def _score_segment_motion(
    *,
    temporal_jitter: float | None,
    spatial_inconsistency: float | None,
    angle_dispersion: float | None,
    flow_mag_mean: float | None,
    cohort: dict[str, float],
) -> float | None:
    required = (temporal_jitter, spatial_inconsistency, angle_dispersion, flow_mag_mean)
    if not cohort or any(value is None for value in required):
        return None
    aggregate = {
        "temporal_jitter": float(temporal_jitter),
        "spatial_inconsistency_mean": float(spatial_inconsistency),
        "angle_dispersion_mean": float(angle_dispersion),
        "flow_mag_mean": float(flow_mag_mean),
    }
    try:
        return _motion_anomaly_score(aggregate, cohort)
    except KeyError:
        return None
...
def build_motion_timeline(
    ...
    if not per_pair:
        return []
    if not segments:
        segments = _build_segments(per_pair, segment_count=DEFAULT_SEGMENT_COUNT)

    quality_metadata = report.get("quality_metadata") or report.get("qualityMetadata") or {}
    video_metadata = report.get("video_metadata") or {}
    fps = quality_metadata.get("fps") or video_metadata.get("fps")

    timeline: list[dict[str, Any]] = []
    for segment in segments:
        pair_start = int(segment.get("pair_start") or 0)
        pair_end = int(segment.get("pair_end") if segment.get("pair_end") is not None else pair_start)
        chunk = per_pair[pair_start : pair_end + 1]
        if not chunk:
            continue

        flow_mag_mean = _segment_metric_mean(segment, "flow_mag_mean")
        spatial_inconsistency = _segment_metric_mean(segment, "spatial_inconsistency")
        angle_dispersion = _segment_metric_mean(segment, "angle_dispersion")
        temporal_jitter = _temporal_jitter(chunk)
        score = _score_segment_motion(
            temporal_jitter=temporal_jitter,
            spatial_inconsistency=spatial_inconsistency,
            angle_dispersion=angle_dispersion,
            flow_mag_mean=flow_mag_mean,
        ...
        if score is not None and score >= threshold:
            warnings.append("HIGH_MOTION_INSTABILITY")
        if fps is None:
            warnings.append("MISSING_FPS")

        timeline.append(
            {
                "segmentIndex": segment.get("segment_index"),
                "startMs": _frame_to_ms(chunk[0].get("frame_index_start"), fps),
                "endMs": _frame_to_ms(chunk[-1].get("frame_index_end"), fps),
                "motionInstabilityScore": _round_optional(score),
                "flow_mag_mean": _round_optional(flow_mag_mean),
                "temporal_jitter": _round_optional(temporal_jitter),
                "spatial_inconsistency": _round_optional(spatial_inconsistency),
                "angle_dispersion": _round_optional(angle_dispersion),
...
def top_suspicious_motion_segments(
    motion_timeline: list[dict[str, Any]],
    limit: int = 3,
) -> list[dict[str, Any]]:
    scored = [
        segment
        for segment in motion_timeline
        if _as_float_or_none(segment.get("motionInstabilityScore")) is not None
    ]
    return sorted(
        scored,
        key=lambda segment: float(segment.get("motionInstabilityScore") or 0.0),
        reverse=True,
    )[:limit]
...
def _build_benchmark_item(
    ...
    is_gmflow = is_gmflow_model(report.get("model") or model_name)
    item: dict[str, Any] = {
    ...
    if status != "ok":
        ...
        if is_gmflow:
            evidence = report.get("gmflowAuxiliaryEvidence") or build_gmflow_auxiliary_evidence(
                raw_score=None,
                quality_metadata=report.get("quality_metadata") or report.get("qualityMetadata") or {},
                motion_timeline=report.get("motionTimeline") or [],
                suspicious_segments=report.get("suspiciousMotionSegments") or [],
            ...
            item.update(
                {
                    ...
                    "gmflowStatus": evidence["gmflowStatus"],
                    "qualityStatus": evidence["qualityStatus"],
                    "contributesToFinalScore": False,
                    "gmflowRawScore": None,
                    "motionInstabilityScore": None,
                    "motionInstabilityLevel": evidence["motionInstabilityLevel"],
                    "scoreMeaning": evidence["scoreMeaning"],
                    "display": evidence["display"],
                    "explanation": evidence["explanation"],
                    "limitations": evidence["limitations"],
                    "notes": evidence["notes"],
                    "qualityMetadata": evidence["qualityMetadata"],
                    "qualityMetadataMissing": evidence["qualityMetadataMissing"],
                    "qualityGateReasons": evidence["qualityGateReasons"],
                    "motionTimeline": evidence["motionTimeline"],
                    "suspiciousMotionSegments": evidence["suspiciousMotionSegments"],
        ...
        item.update(
            {
                **common_fields,
                ...
                "qualityStatus": evidence["qualityStatus"],
                "contributesToFinalScore": False,
                "gmflowRawScore": evidence["gmflowRawScore"],
                "motionInstabilityScore": evidence["motionInstabilityScore"],
                "motionInstabilityLevel": evidence["motionInstabilityLevel"],
                "scoreMeaning": evidence["scoreMeaning"],
                "display": evidence["display"],
                "explanation": evidence["explanation"],
...
def build_benchmark_report_document(
    ...
    *,
    ...
    document = {
        ...
        "metrics": metrics,
        "items": [_build_benchmark_item(item, threshold, cohort, model_name=model_name) for item in report_list],
...
_METRIC_GLOSSARY: dict[str, tuple[str, str]] = {
    "motion_anomaly_score": ("motion instability score", "auxiliary motion evidence, not fake probability"),
    "motionInstabilityScore": ("motion instability score", "auxiliary motion evidence, not fake probability"),
...
_OVERALL_METRICS_KO: dict[str, tuple[str, str]] = {
    "heuristic_accuracy": ("diagnostic threshold match", "threshold match rate for analysis only; not final accuracy"),
    "motion_anomaly_score_mean_fake": ("fake mean motion instability", "mean auxiliary motion instability score for fake"),
    "motion_anomaly_score_mean_real": ("real mean motion instability", "mean auxiliary motion instability score for real"),
...
def _render_file_body(item: dict[str, Any], threshold: float) -> str:
    ...
    is_gmflow = item.get("modelName") == GMFLOW_MODEL_NAME or is_gmflow_model(item.get("model"))
    gt = item.get("ground_truth_label")
    pred = item.get("pred_label")
    if is_gmflow:
        role_text = item.get("role") or GMFLOW_ROLE
        status_text = item.get("gmflowStatus") or GMFLOW_AUXILIARY_STATUS
        processing_status = item.get("processingStatus") or processing_status_from_report_status(status)
        quality_status = item.get("qualityStatus") or GMFLOW_QUALITY_AVAILABLE_STATUS
        instability_level = item.get("motionInstabilityLevel") or "-"
        contributes = str(item.get("contributesToFinalScore")).lower()
        reasons = item.get("qualityGateReasons") or []
        reason_text = ", ".join(str(x) for x in reasons) if reasons else "none"
        parts: list[str] = [
            f'<div class="verdict">'
            f'<div class="verdict-row"><span>truth</span>{_label_badge(gt)}</div>'
            f'<div class="verdict-row"><span>role</span><strong>{html.escape(str(role_text))}</strong></div>'
            f'<div class="verdict-row"><span>processingStatus</span><strong>{html.escape(str(processing_status))}</strong></div>'
            f'<div class="verdict-row"><span>gmflowStatus</span><strong>{html.escape(str(status_text))}</strong></div>'
            f'<div class="verdict-row"><span>qualityStatus</span><strong>{html.escape(str(quality_status))}</strong></div>'
            f'<div class="verdict-row"><span>motionInstabilityScore</span>'
            f'<strong>{_fmt_num(item.get("motionInstabilityScore"))}</strong>'
            f' <span class="note">({html.escape(str(instability_level))}, auxiliary evidence only)</span></div>'
            f'<div class="verdict-row"><span>contributesToFinalScore</span><strong>{contributes}</strong></div>'
            f"</div>",
            f'<p class="note">{html.escape(str(item.get("explanation") or GMFLOW_EXPLANATION))}</p>',
            f'<p class="note">quality gate reasons: {html.escape(reason_text)}</p>',
        ]
    else:
    ...
    if is_gmflow:
        parts.append(
            f'<p class="note">diagnostic motion threshold votes: high {votes.get("fake", 0)}, '
            f'normal {votes.get("real", 0)} (frames_used={item.get("frames_used")}, '
            f'pairs={item.get("frame_pairs_used")})</p>'
        )
    else:
        parts.append(
    ...
    if is_gmflow:
        suspicious_segments = item.get("suspiciousMotionSegments") or []
        motion_timeline = item.get("motionTimeline") or []
        if suspicious_segments:
            rows = [
                [
                    ...
                    segment.get("startMs"),
                    segment.get("endMs"),
                    segment.get("motionInstabilityScore"),
                    segment.get("warning") or "-",
                ]
                for segment in suspicious_segments
            ]
            parts.append("<h4>suspicious motion segments</h4>")
            parts.append(_render_simple_table(["segment", "startMs", "endMs", "motion", "warning"], rows))
        if motion_timeline:
            rows = [
                [
                    segment.get("segmentIndex"),
                    ...
                    segment.get("endMs"),
                    segment.get("motionInstabilityScore"),
                    segment.get("flow_mag_mean"),
                    segment.get("temporal_jitter"),
                    segment.get("warning") or "-",
                ]
                for segment in motion_timeline
            ]
            parts.append(
                '<details class="sub"><summary>motion timeline</summary><div class="sub-body">'
                + _render_simple_table(
                    ["segment", "startMs", "endMs", "motion", "flow mean", "temporal jitter", "warning"],
                    rows,
                )
                + "</div></details>"
            )
...
def generate_benchmark_html(document: dict[str, Any]) -> str:
    ...
    for item in items:
        ...
        pred = item.get("pred_label")
        is_gmflow = item.get("modelName") == GMFLOW_MODEL_NAME or is_gmflow_model(item.get("model"))
        score = item.get("motionInstabilityScore") if is_gmflow else item.get("fake_score")
        status = item.get("status")
        correct = item.get("correct")
        ...
        if status != "ok":
            ...
            title = (
                f"{html.escape(file_name)} | truth {_label_badge(gt)} | "
                f"motion instability {_fmt_num(score)} | "
                f"{html.escape(str(item.get('gmflowStatus') or GMFLOW_AUXILIARY_STATUS))}"
            )
        else:
...
<body>
  <div class="panel">
    ...
    <p class="note">
      Optical-flow <strong>motion instability evidence</strong>.
      Diagnostic threshold metrics are not final fake/real classifier performance.
    </p>
  </div>
  <div class="panel">
    <h2>Overall</h2>
